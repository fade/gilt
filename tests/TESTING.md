...
**Legend:** `[P]` Pass | `[F]` Fail | `[S]` Skip | `[N/A]` Not applicable
...
|---|------|-----|----------|--------|-------|
| 1.1 | Launch gilt | `gilt` | TUI renders with 5 left panels + main + cmdlog | [ ] | |
| 1.2 | Status panel shows repo name | — | "[1] Status" with repo name and branch | [ ] | |
| 1.3 | Files panel shows files | — | Modified, staged, untracked files visible | [ ] | |
| 1.4 | Branches panel shows branches | — | master (green *), feature branches listed | [ ] | |
| 1.5 | Commits panel shows history | — | Commits with yellow hash, initials, message | [ ] | |
| 1.6 | Stash panel shows stashes | — | 2 stashes from setup script | [ ] | |
| 1.7 | Help bar visible | — | Context-sensitive hints at bottom | [ ] | |
| 1.8 | Main panel shows diff | — | Diff for selected file in right panel | [ ] | |
| 1.9 | Command log visible | — | Bottom-right shows recent commands | [ ] | |
| 1.10 | Recent repo saved | — | Check `~/.config/gilt/recent-repos.txt` exists | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 3.1 | Stage a file | `Space` on unstaged | File moves to staged, indicator changes | [ ] | |
| 3.2 | Unstage a file | `Space` on staged | File moves to unstaged | [ ] | |
...
| 3.5 | Diff updates on select | `j`/`k` | Main panel diff changes per selected file | [ ] | |
...
| 5.4 | Blame view | `b` | Main panel shows blame: hash, author, line | [ ] | |
| 5.5 | Blame commit detail | `Enter` in blame | Dialog shows commit details for line | [ ] | |
| 5.6 | Exit blame | `Escape` | Returns to normal view | [ ] | |
| 5.7 | File tree toggle | `T` | Files grouped by directory with tree indent | [ ] | |
...
| 5.11 | Ignore file | `I` | Dialog asks to add to .gitignore, confirm adds it | [ ] | |
| 5.12 | Verify .gitignore | — | Check .gitignore contains the file | [ ] | |
| 5.13 | Copy file path | `y` | Cmdlog shows "Copied file path: ..." | [ ] | |
| 5.14 | Verify clipboard | paste | Pasted text matches file path | [ ] | |
| 5.15 | External diff tool | `x` | TUI suspends, difftool opens, TUI restores | [ ] | |
...
| 6.5 | Stash pop | `P` on stash | Stash applied and removed from list | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 7.1 | Checkout branch | `Enter` | Branch checked out, status updates | [ ] | |
| 7.2 | New branch | `n` | Input dialog, branch created and checked out | [ ] | |
| 7.3 | Rename branch | `N` | Input dialog, branch renamed | [ ] | |
| 7.4 | Delete branch | `D` | Confirmation, branch deleted | [ ] | |
...
| 7.14 | Copy branch name | `y` | Cmdlog shows "Copied branch name: ..." | [ ] | |
| 7.15 | Open in browser | `o` | Browser opens branch URL | [ ] | |
| 7.16 | Create PR | `O` | Browser opens compare URL for PR | [ ] | |
...
| 8.6 | Fetch | `f` | Fetch dialog, fetches from selected remote | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 9.1 | Cycle to Tags | `w` | Panel title changes to "Tags" | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 10.1 | Cycle to Submodules | `w` | Panel title changes to "Submodules" | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 11.1 | View commit detail | select commit | Main panel shows hash, author, date, message | [ ] | |
...
| 11.13 | Copy commit hash | `y` | Cmdlog shows "Copied commit hash: ..." | [ ] | |
| 11.14 | Open in browser | `o` | Browser opens commit URL | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 14.1 | Create stash | `s` | Dialog, stash created | [ ] | |
| 14.2 | Pop stash | `g` | Top stash applied and removed | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 17.1 | Push | `P` | Push to remote, cmdlog shows result | [ ] | |
| 17.2 | Pull | `p` | Pull from remote, cmdlog shows result | [ ] | |
| 17.3 | Fetch | `f` on branches | Fetch dialog, fetches from remote | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 19.1 | Conflict detected | — | Status shows MERGING, files show `!` | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 21.1 | Copy file path | `y` on files | Clipboard has file path | [ ] | |
| 21.2 | Copy branch name | `y` on branches | Clipboard has branch name | [ ] | |
| 21.3 | Copy commit hash | `y` on commits | Clipboard has short hash | [ ] | |
| 21.4 | Open commit URL | `o` on commits | Browser opens commit page | [ ] | |
| 21.5 | Open branch URL | `o` on branches | Browser opens branch page | [ ] | |
...
| 24.2 | Config file exists | — | `~/.config/gilt/recent-repos.txt` has entries | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 25.1 | Open git-flow menu | `E` | Dialog with flow options | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 26.1 | Create PR | `O` on branch | Browser opens compare URL | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 27.1 | Custom command runs | `9` | Cmdlog shows `$ git log --oneline -5` + output | [ ] | |
...
| 28.2 | Confirm ignore | Ignore button | File added to .gitignore | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 29.1 | Launch difftool | `x` on modified file | TUI suspends, difftool opens | [ ] | |
...
| # | Test | Key | Expected | Result | Notes |
|---|------|-----|----------|--------|-------|
| 30.1 | Navigate to submodules | `3` → `w` ×3 | Submodules view shown | [ ] | |
