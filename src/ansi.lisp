...
;; Git-specific colors
(register-color :diff-add 34)
(register-color :diff-remove 160)
(register-color :diff-header 75)
(register-color :diff-hunk 140)
...
(defun cursor-up (&optional (n 1))
  (format t "~C[~DA" *escape* n))

(defun cursor-down (&optional (n 1))
  (format t "~C[~DB" *escape* n))

(defun cursor-forward (&optional (n 1))
  (format t "~C[~DC" *escape* n))

(defun cursor-back (&optional (n 1))
  (format t "~C[~DD" *escape* n))

;;; Legacy convenience functions for compatibility - use *terminal-io*
...
(defun color-code (name)
  "Get color code from name or return number as-is (legacy compatibility)"
  (etypecase name
    (integer name)
