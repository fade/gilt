    ...
    (cond
      ...
      ((member "--debug" args :test #'string=)
       (let ((tty-path (namestring gilt.terminal:*tty-path*)))
         ...
         (format t "~%Checking terminal size (ioctl)...~%")
         (handler-case
             (let ((size (gilt.terminal:terminal-size)))
               (format t "  size: ~Dx~D~%" (first size) (second size)))
           (error (e)
             (format t "  size: FAILED - ~A~%" e)))
         (format t "~%Testing raw mode (termios FFI)...~%")
         (handler-case
             (progn
               (gilt.terminal:enable-raw-mode gilt.terminal:*terminal-mode*)
               (format t "  raw mode: OK~%")
               (gilt.terminal:disable-raw-mode gilt.terminal:*terminal-mode*))
           (error (e)
             (gilt.terminal:disable-raw-mode gilt.terminal:*terminal-mode*)
